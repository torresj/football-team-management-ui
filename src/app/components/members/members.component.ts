...
import { BlockMemberComponent } from '../dialogs/block-member/block-member.component';
...
export class MembersComponent implements OnInit {
        ...
        this.responsive.observe(['(min-width: 700px)']).subscribe({
          next: (value) => {
            if (value.matches) {
              ...
              this.columns = this.isAdmin.value
                ? this.columnsToDisplayInSmallScreensAdmin
                : this.columnsToDisplayInSmallScreens;
            }
          },
        });
      },
  ...
  createMember() {
    ...
    this.dialog
      .open(CreateMemberComponent, dialogConfig)
      .afterClosed()
      .subscribe({
  ...
  deleteMember(member: Member) {
    ...
    this.dialog
      .open(DeleteMemberComponent, dialogConfig)
      .afterClosed()
      .subscribe({
        next: () => this.getMembers(),
      });
  ...
  injuredMember(member: Member) {
    ...
    this.dialog
      .open(InjuredMemberComponent, dialogConfig)
      .afterClosed()
      .subscribe({
        next: () => this.getMembers(),
      });
